...
namespace FreeswitchListenerServer.InboundApi.QueueApp
{
    ...
    /// به ترتیب
    /// </summary>
    internal class SequentiallyByAgentOrder : AppQueueCall
    {
        /// <summary>
        /// به ترتیب
        /// </summary>
            ...
            List<vwComQueueAgent> queueAgentList)
        {
            ...
            var orderedQueueAgentCount = queueAgentList.Count;
            var queueAgentIdList = queueAgentList.Select(p => p.ComQueueAgentPID).ToList();
            var doBreak = false;
            var doAgain = false;
            queueAgentList.CustomeForEach(ref doBreak, ref doAgain, async (queueAgent, queueAgentIndex) =>
            {
                    ...
                    if (queueAgentIndex == orderedQueueAgentCount - 1)
                    {
                        ...
                        if (resetAfterMaxCallForAllAgents)
                        {
                            ...
                            if (noFinishedQueueAgentCallIdList.Count > 0)
                            {
                                ...
                                queueAgentList = queueAgentList.Where(p =>
                                    noFinishedQueueAgentCallIdList.Contains(p.ComQueueAgentPID)).ToList();

                                orderedQueueAgentCount = queueAgentList.Count;
                            }
                            else
                            {
                ...
                if (queueAgentIndex == orderedQueueAgentCount - 1 && !channel.IsAnswered)
                {
